...
def add_update_tab():
    ...
    with st.form(key="expenses_form"):
        ...
        for i in range(5):
            ...
            with col2:
                category_input = st.selectbox(label="Cetegory", options=categories, index=categories.index(category) ,key=f"categories_{i}", label_visibility="collapsed")
            with col3:
        ...
        if submit:
            ...
            if response.status_code == 200:
                st.success("Expense updates successfully!")
            else:
                st.error("Something went wrong")
